...
In <GmlFilename filename={choicerCreateName} />, the flag's value will be set to 0.

In <GmlFilename filename={choicerStepName} />, the flag's value will be incremented by 1 every frame.
...
This means that the flag might be reused for some other time-based choicers in future chapters, or that the code in the <span class="text-green">Chapter 4</span> choicers were just copy pasted.

The only interaction in the game currently where the time spent on the choicer is relevant is in an interaction with <span class="text-soul">Sans</span> in his shop, when asking Kris to come over the next day.
